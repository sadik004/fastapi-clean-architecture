...
def test_repository_create_and_get() -> None:
    ...
    assert repo.get_by_username("test_user") == user

    # Non-existent lookups
    assert repo.get_by_id(999) is None
    assert repo.get_by_email("unknown@example.com") is None
    assert repo.get_by_username("unknown_user") is None


def test_repository_update_synchronizes_indexes() -> None:
    ...
    assert repo.get_by_email("old_email@example.com") is None
    assert repo.get_by_username("old_handle") is None

    # New keys must be mapped in secondary inverted indexes
    assert repo.get_by_email("new_email@example.com") == updated
    assert repo.get_by_username("new_handle") == updated


def test_repository_update_non_existent() -> None:
    """Verify updating a non-existent user returns None."""
    repo = InMemoryUserRepository()
    assert repo.update(user_id=999, age=30) is None


def test_repository_delete_purges_secondary_indexes() -> None:
    ...
    assert repo.get_by_email("purge@example.com") is None
    assert repo.get_by_username("purge_me") is None

    # Critical invariant: subsequent creation with freed email & username succeeds
    recreated_user = repo.create(
        email="purge@example.com",
        username="purge_me",
        password_hash="new_hash",
    )
    assert recreated_user.id == 2
    assert repo.get_by_email("purge@example.com") == recreated_user
    assert repo.get_by_username("purge_me") == recreated_user


def test_repository_delete_non_existent() -> None:
    """Verify deleting non-existent ID returns False without error."""
    repo = InMemoryUserRepository()
    assert repo.delete(9999) is False


def test_repository_pagination_slicing() -> None:
    ...
    page_empty = repo.list_all(limit=2, offset=10)
...
def test_repository_clear() -> None:
    ...
    repo = InMemoryUserRepository()
    repo.create(email="clear@example.com", username="clear_user", password_hash="h")
    repo.clear()

    assert repo.list_all() == []
    assert repo.get_by_email("clear@example.com") is None
    assert repo.get_by_username("clear_user") is None
