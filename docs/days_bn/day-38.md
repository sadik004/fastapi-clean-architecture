...
**টোকেন বাকেট অ্যালগরিদম (Token Bucket Algorithm) ও রেডিস অ্যাটমিক লুয়া স্ক্রিপ্টিং (EVALSHA) ($\mathcal{O}(1)$ Time, $\mathcal{O}(1)$ Space)**

## 🚀 প্রোডাকশনে ঠিক কখন ব্যবহার করব? (When to use in Production)
- **পাবলিক ক্লাউড এপিআই গেটওয়ে (যেমন AWS, Stripe, GitHub API):** যেখানে ক্লায়েন্ট বা ব্রাউজারের স্বাভাবিক ট্রাফিক বাস্ট (Burst) হ্যান্ডেল করতে হয়।
- **ই-কমার্স চেকআউট বা ফ্ল্যাশ সেল:** ইউজার পেজ লোডের সময় একসাথে ৪–৫টি এপিআই কল করলেও যেন তা সুন্দরভাবে পাস হতে পারে।
- **আল্ট্রা-লাইটওয়েট রেট লিমিটিং:** যেখানে কোটি কোটি ইউজারের রেট লিমিট হিসাব রাখতে মেমোরি খরচ নূন্যতম রাখতে হয়।
- **হাইজিপিএস ও আল্ট্রা-হাই থ্রুপুট মাইক্রোসার্ভিস ক্লাস্টার (100k+ Active Clients):** যেখানে প্রতি সেকেন্ডে লাখ লাখ রিকোয়েস্ট আসে, সেখানে ডে ৩৭-এর স্লাইডিং উইন্ডো লগে প্রতি রিকোয়েস্টের টাইমস্ট্যাম্প ZSET-এ জমালে কয়েক গিগাবাইট র‍্যাম নষ্ট হয়। টোকেন বাকেটে ক্লায়েন্ট লাখ রিকোয়েস্ট পাঠালেও মেমোরি খরচ সর্বদা কনস্ট্যান্ট $\mathcal{O}(1)$ (< ১০০ বাইট)!
- **পেমেন্ট চেকআউট ও পেমেন্ট প্রসেসিং গেটওয়ে:** যেখানে একাধিক সার্ভার নোড থেকে একই ক্লায়েন্টের ব্যালেন্স বা কোটা কাটার সময় কোনো ধরনের রেস কন্ডিশন সহ্য করা যায় না।

## 🎯 কী কারণে বা কোন পরিস্থিতিতে ব্যবহার করব? (Why to use / Technical Triggers)
- **স্লাইডিং উইন্ডোর চেয়ে ৯০%+ মেমোরি সাশ্রয় ($\mathcal{O}(1)$ Space):** প্রতি ক্লায়েন্টের জন্য ZSET-এ শত শত টাইমস্ট্যাম্প জমানোর বদলে রেডিস হ্যাশে মাত্র ২টি সংখ্যা (`tokens` ও `last_updated`) রাখা (< ১০০ বাইট)।
...
- **মসৃণ ট্রাফিক শেপিং (Traffic Shaping):** শান্ত সময়ে জমানো টোকেন দিয়ে বাস্ট হ্যান্ডেল করা, আবার টানা চাপ থাকলে রিফিল রেটে (r) গতি বেঁধে ফেলা।
