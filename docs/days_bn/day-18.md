...
- **Exponential Backoff with Full Jitter & Linear Retry Loop**: ক্ষণস্থায়ী নেটওয়ার্ক ফল্ট রিকভারির জন্য সূচকীয় বিলম্ব অ্যালগরিদম ($t = \text{base} \times 2^{\text{attempt}} + \text{jitter}$)।

### 🚀 প্রোডাকশনে ঠিক কখন ব্যবহার করব? (When to use in Production)
- **ক্লাউড ডেটাবেস ফেলওভার ও হাই-অ্যাভেইলেবিলিটি সুইচ**: AWS RDS Multi-AZ বা Aurora ফেলওভারের সময় ড্রপড কানেকশন রিকভারি।
- **ক্ষণস্থায়ী নেটওয়ার্ক ব্লিঙ্ক ও সকেট ড্রপ**: ডাটাবেস ও ব্যাকএন্ডের মাঝের নেটওয়ার্কে সাময়িক মাইক্রোসেকেন্ড ড্রপ হ্যান্ডলিং।
- **স্পাইক ট্রাফিকে ডাটাবেস ব্যাকপ্রেশার ম্যানেজমেন্ট**: লোড হঠাৎ বেড়ে গিয়ে সাময়িক কানেকশন রিফিউজাল আসলে ব্যাকঅফ করে রিকোয়েস্ট বাঁচানো।

### 🎯 কী কারণে বা কোন পরিস্থিতিতে ব্যবহার করব? (Why to use / Technical Triggers)
- **ট্রানজিয়েন্ট নেটওয়ার্ক এররে ইউজারকে 500 ইন্টারনাল সার্ভার এরর দেখানো বন্ধ**: সামান্য নেটওয়ার্ক ব্লিঙ্কে রিকোয়েস্ট ফেইল না করে ব্যাকগ্রাউন্ডে স্বয়ংক্রিয়ভাবে রিট্রাই করে সফল রেসপন্স প্রদান।
- **ক্যাসকেডিং থান্ডারিং হার্ড (Thundering Herd) প্রতিরোধ**: সব ফেইল্ড রিকোয়েস্ট একই সাথে ডাটাবেসে ধাক্কা দিলে ডাটাবেস স্থায়ীভাবে ডাউন হয়ে যায়; Exponential Backoff with Jitter অনুরোধগুলোকে মসৃণভাবে ছড়িয়ে দেয়।
- **সার্কিট ব্রেকিং ইন্টিগ্রেশন**: ডাটাবেস স্থায়ীভাবে ডাউন থাকলে বারবার আঘাত না করে দ্রুত ফেইল-ফাস্ট নিশ্চিত করা।
