...
**অপটিমিস্টিক কনকারেন্সি কন্ট্রোল (OCC) — Row Version Stamping with Atomic Conditional UPDATE ($\mathcal{O}(1)$ DB Write, Zero Lock Overhead)**

## 🚀 প্রোডাকশনে ঠিক কখন ব্যবহার করব? (When to use in Production)
- **ব্যাংক অ্যাকাউন্ট ব্যালেন্স আপডেট:** যেখানে একই সময়ে দুটি ট্রানজেকশন একই অ্যাকাউন্টের ব্যালেন্স আপডেট করার চেষ্টা করে এবং একটি আপডেট অন্যটিকে মুছে দিতে পারে।
- **ই-কমার্স প্রোডাক্ট স্টক ম্যানেজমেন্ট:** ফ্ল্যাশ সেলে হাজারো কনকারেন্ট ইউজার একই প্রোডাক্টের স্টক কাউন্ট কমানোর চেষ্টা করলে Overselling প্রতিরোধ।
- **হেলথকেয়ার প্যাশেন্ট রেকর্ড:** দুই ডাক্তার একই রোগীর প্রেসক্রিপশন একই সময়ে সম্পাদনা করার চেষ্টা করলে একজনের পরিবর্তন হারিয়ে না যাওয়া নিশ্চিত করা।
- **কোলাবোরেটিভ ডকুমেন্ট এডিটিং:** Wiki, CMS, বা কনফিগ ম্যানেজমেন্ট সিস্টেমে কনকারেন্ট এডিটরদের মধ্যে কনফ্লিক্ট ডিটেকশন।
- **সব এন্টারপ্রাইজ CRUD API:** যেখানে ডেটা কনসিস্টেন্সি সর্বোচ্চ অগ্রাধিকার এবং ডেটাবেস লক দিয়ে থ্রুপুট কমানো গ্রহণযোগ্য নয়।

## 🎯 কী কারণে বা কোন পরিস্থিতিতে ব্যবহার করব? (Why to use / Technical Triggers)
- **Lost Update Problem:** দুটি ট্রানজেকশন একই সারি পড়ে (`version=5`), দুটোই পরিবর্তন করে, শেষেরটি আগেরটির পরিবর্তন সম্পূর্ণ মুছে দেয়—একটি নীরব, বিধ্বংসী ডেটা দুর্নীতি।
- **পেসিমিস্টিক লকের বিপদ (SELECT FOR UPDATE):** ভারী ডেটাবেস লক কম কনকারেন্সি লোডেও ডেডলক, থ্রেড স্ট্যার্ভেশন, এবং ১০×–১০০× থ্রুপুট ড্রপ ঘটায়।
- **অ্যাটমিক কন্ডিশনাল আপডেটের প্রয়োজনীয়তা:** SQL `UPDATE WHERE id = X AND version = Y` একটি একক, আনডিভাইডেবল অপারেশন যা কোনো অ্যাপ্লিকেশন-লেভেল লক ছাড়াই ACID আইসোলেশন গ্যারান্টি দেয়।
